...
export function AnomalyReviewBoard() {
  ...
  const rows = useMemo(() => {
    const current = anomalies.length > 0 ? anomalies : sourceAnomalies;
    return [...current].sort((left, right) => right.exposureMad - left.exposureMad);
  }, [anomalies, sourceAnomalies]);

  function decide(id: string, status: "validated" | "rejected") {
    setAnomalies((current) => {
      const base = current.length > 0 ? current : sourceAnomalies;
